...
class PosCorrectionNetwork(torch.nn.Module):
    ...
    def __init__(
        ...
        self.timestep = TimestepPredictionNetwork()
        if timestep_net:
            weights = torch.load(timestep_net)
            self.timestep.load_state_dict(weights)
            self.timestep.requires_grad_(False)

        gravity = torch.FloatTensor([0, -9.81, 0])
        self.register_buffer('gravity', gravity)
        self._all_convs = []
        self.neighbor_result = []
    ...
    def integrate_pos_vel(self, pos1, vel1, timestep):
    ...
    def compute_new_pos_vel(self, pos1, vel1, pos2, vel2, pos_correction, timestep):
    ...
    def compute_correction(self,
        ...
        self.neighbor_result.append(self.num_fluid_neighbors)
    ...
    def forward(self, inputs, fixed_radius_search_hash_table=None):
        ...
        with torch.no_grad():
            timestep = float(self.timestep(inputs))
        
        pos2, vel2 = self.integrate_pos_vel(pos, vel, timestep)
        ...
        pos2_corrected, vel2_corrected = self.compute_new_pos_vel(
            pos, vel, pos2, vel2, pos_correction, timestep)
