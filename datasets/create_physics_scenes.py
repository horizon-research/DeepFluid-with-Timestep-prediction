...
def obj_surface_to_particles(objpath, radius=None):
    ...
    print('num_points of wall', num_points)
...
def create_fluid_data(output_dir, seed, options):
    ...
    bounding_boxes = sorted(glob(os.path.join(SCRIPT_DIR, 'models', 'Box*.obj')))
    ...
    print("number of fluid shapes", len(fluid_shapes))
    ...
    for create_scene_i in range(100):
        print("chance for creating scenes ", create_scene_i)
        if scene_is_valid:
            break
        ...
        print("total number of fluid particles: ", total_number_of_fluid_particles)
        if options.const_fluid_particles:
            if options.const_fluid_particles > total_number_of_fluid_particles:
                ...
                while get_total_number_of_fluid_particles(
                    ...
                    print("not enough, add more into objects")
    ...
    rigid_body = deepcopy(default_rigidbodies)
    ...
    for obj in objects:
        ...
        fluid_count += 1
        # fluid = deepcopy(default_fluidmodel)
