...
class FluidErrors:
    ...
    def add_errors(self,
        ...
        if np.count_nonzero(~np.isfinite(predicted)):
            print('predicted_pos contains nonfinite values')
            return

        if np.count_nonzero(~np.isfinite(gt)):
            print('gt_pos contains nonfinite values')
            return
        
        if self.flag == 'ts':
            errs = _ts_distance(predicted, gt)
        else:
            errs = _compute_stats(_distance(predicted, gt))
